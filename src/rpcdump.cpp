...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <ussrcoinprivkey> [label] [rescan=true]\n"
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Ussrcoin address or script");
...
Value dumppem(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "dumppem <ussrcoinaddress> <filename> <passphrase>\n"
            "Dump the key pair corresponding to <ussrcoinaddress> and store it as encrypted PEM file."
            + HelpRequiringPassphrase());
