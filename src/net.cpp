...
static const char *strDNSSeed[][2] = {
    {"ussr-unit.su", "seed5.ussr-unit.su"},
    {"ussr-unit.su", "blockchain.ussr-unit.su"},
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("ussrunit-dnsseed");
...
uint32_t pnSeed[] =
{
0x7f000001,0x05bce52b,0x05bcdf5a,0xb976a46d
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. UssrUnit is probably already running."), addrBind.ToString().c_str());
        else
