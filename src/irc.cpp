...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ussrcoinTEST2\r");
            Send(hSocket, "WHO #ussrcoinTEST2\r");
        } else {
            // randomly join #ussrcoin00-#ussrcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ussrunit%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ussrunit%02d\r", channel_number).c_str());
        }
