...
def confFile():
    ...
    if platform.system() == 'Windows':
        folder = os.path.join(os.path.join(os.environ['APPDATA'], 'UssrUnit'))
    else:
        if platform.system() == 'Darwin':
            folder = os.path.expanduser('~/Library/Application Support/UssrUnit/')
        else:
            folder = os.path.expanduser('~/.ussrunit')

    return os.path.join(folder, 'ussrunit.conf')

conf_path = confFile()
if not os.path.exists(conf_path):
    parser.error('''Ussrcoin configuration file not found. Manually enter your RPC password.\r\n'''
        ...
        '''rpcpassword=%x\r\n'''
        '''\r\n'''
        '''Keep that password secret! After creating the file, restart Ussrcoin.''' % (conf_path, random.randrange(2**128)))
...
for node in json.loads(data.decode()):
    ...
    try:
        ...
        if e.errno == errno.ECONNREFUSED:
            print ('Unable to communicate with Ussrcoin RPC')
        break
